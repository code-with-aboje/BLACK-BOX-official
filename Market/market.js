...
// 🆕 PURCHASE FUNCTION WITH TOKEN TRANSFER
async function purchaseTool(toolId, toolPrice, sellerId, toolName) {
  ...
  try {
    ...
    console.log(`💸 Transferred ${toolPrice} tokens from buyer to seller`);
    console.log(`💰 Buyer new balance: ${newBuyerBalance}`);
...
onValue(ref(db, "tools"), async (snapshot) => {
  ...
  if (snapshot.exists()) {
    ...
    allTools.sort((a, b) => (b.uploadedAt || 0) - (a.uploadedAt || 0));
    ...
    for (const tool of allTools) {
      ...
      console.log(`👤 Tool: ${tool_name} | Uploader: ${uploaderName} | Time: ${timeAgo} | Downloads: ${downloadCount}`);
      ...
      price.innerHTML = `
      <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: #00ff9c;">
      ...
      download_info.innerHTML = `
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#00ff7f" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      info_content.appendChild(usage_title);
      ...
      content.style.cssText = `
        ...
        overflow-y: auto;
      `;
      content.textContent = usage || "No description available.";
      info_content.append(content);
      

      const info_footer = document.createElement("div");
      info_footer.style.cssText = `
        margin-top: 15px;
        padding-top: 10px;
      ...
      if (!document.getElementById('popup-animation-style')) {
        popupStyle.id = 'popup-animation-style';
      ...
      // 🆕 MODIFIED UNLOCK BUTTON WITH PURCHASE LOGIC
      btn_unlock.addEventListener('click', async () => {
       window.location.href = '../Bank/bank.html';
      });

      if (isPurchased) {
        btn_unlock.style.display = 'none';
        btn_download.style.background = '#00ff7f44';
        btn_download.style.color = '#00ff7f';
        ...
        btn_download.addEventListener('click', async () => {
          ...
          console.log('Download triggered for:', tool_id);
        });
        
        btn_download.onmouseenter = () => (btn_download.style.background = "#00ff7f66");
        btn_download.onmouseleave = () => (btn_download.style.background = "#00ff7f44");
      } else {
        btn_download.style.background = '#333';
        btn_download.style.color = '#666';
        btn_download.style.cursor = 'not-allowed';
        btn_download.style.opacity = '0.5';
        btn_download.disabled = true;
        btn_download.textContent = '🔒 Locked';
      ...
      btn_download.style.height = '20px';
      btn_download.style.lineHeight = '0px';
