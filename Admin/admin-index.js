...
/* ========================================
   ...
   Add these functions to your existing admin-index.js
   ======================================== */
...
// Responsive table rendering wrapper
function renderResponsiveTable(data, renderTableFn, renderMobileCardFn) {
  if (isMobile() && renderMobileCardFn) {
    return renderMobileCardFn(data);
  }
  return renderTableFn(data);
}

// Enhanced mobile card renderer for users
function renderUsersMobileCards(usersList) {
    ...
    <div class="mobile-list">
      ${usersList.map(([userId, user]) => `
        <div class="mobile-card">
          <div class="user-avatar" style="width:56px;height:56px;border-radius:12px;flex-shrink:0;">
            ${user.avatar ? `<img src="${user.avatar}" alt="${user.nickname}" style="width:100%;height:100%;object-fit:cover;border-radius:12px;">` : `
          ...
          <div class="meta">
            ...
            <div class="subtitle">${user.email || 'N/A'}</div>
            <div class="subtitle">Role: ${user.role || 'user'} • <span class="status-badge ${user.status === 'online' ? 'active' : 'inactive'}">${user.status || 'offline'}</span></div>
            ...
            ` : '<div style="color: #888; font-size: 0.85em; margin-top: 8px;">Current User</div>'}
          </div>
        </div>
      `).join('')}
    </div>
...
// Enhanced mobile card renderer for posts
function renderPostsMobileCards(postsList) {
  ...
  return `
    <div class="mobile-list">
      ${postsList.map(([postId, post]) => {
        const author = window.usersData?.[post.authorId] || {};
        return `
          <div class="mobile-card">
            <div class="user-avatar" style="width:56px;height:56px;border-radius:12px;flex-shrink:0;">
              ${author.avatar ? `<img src="${author.avatar}" alt="${author.nickname}" style="width:100%;height:100%;object-fit:cover;border-radius:12px;">` : `
                <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
              `}
            </div>
            <div class="meta">
              <div class="title">${post.title || 'Untitled'}</div>
              <div class="subtitle">${post.content ? (post.content.length > 120 ? post.content.substring(0, 120) + '...' : post.content) : 'No content'}</div>
              <div class="subtitle">By ${author.nickname || 'Anonymous'} • ${post.createdAt ? new Date(post.createdAt).toLocaleDateString() : 'N/A'}</div>
              <div class="subtitle">Status: <span class="status-badge ${post.status || 'pending'}">${post.status || 'pending'}</span></div>
              <div class="actions">
                ${post.status === 'pending' ? `
...
// Enhanced resize handler with re-rendering
let resizeTimeout;
function enhancedHandleResize() {
  ...
  resizeTimeout = setTimeout(() => {
    ...
    if (isMobile()) {
      ...
      if (menuToggle) setAriaExpanded(menuToggle, false);
      
      // Re-render current page to show mobile cards
    ...
    // Clean up focus trap
    if (window.cleanupFocusTrap) {
      window.cleanupFocusTrap();
      window.cleanupFocusTrap = null;
    }
  }, 150);
}
...
function initTouchSwipe() {
  ...
  function handleSwipe() {
    ...
    if (touchStartX < 50 && touchEndX - touchStartX > swipeThreshold) {
      ...
      if (menuToggle) setAriaExpanded(menuToggle, true);
      
      if (window.cleanupFocusTrap) window.cleanupFocusTrap();
      window.cleanupFocusTrap = trapFocus(sidebar);
    }
    
    // Swipe left - close sidebar
    if (sidebar?.classList.contains('mobile-visible') && touchStartX - touchEndX > swipeThreshold) {
      sidebar.classList.remove('mobile-visible');
      if (menuToggle) setAriaExpanded(menuToggle, false);
...
function initMobileSearch() {
  ...
  // On mobile, create a modal search
  if (isMobile()) {
    ...
    searchBtn.onclick = () => {
      ...
      modal.innerHTML = `
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 24px;">
          <input type="text" placeholder="Search..." style="flex: 1; padding: 14px 16px; background: rgba(255, 255, 255, 0.08); border: 1px solid #3a3a3a; border-radius: 12px; color: #fff; font-size: 1em;">
          <button onclick="this.closest('div').parentElement.remove()" style="padding: 14px; background: rgba(255, 255, 255, 0.08); border: 1px solid #3a3a3a; border-radius: 12px; color: #fff;">Cancel</button>
        </div>
    ...
    topBar?.insertBefore(searchBtn, topBar.firstChild);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.renderUsersMobileCards = renderUsersMobileCards;
  window.renderPostsMobileCards = renderPostsMobileCards;
  window.enhancedHandleResize = enhancedHandleResize;
  window.initResponsiveFeatures = initResponsiveFeatures;
}
...
/* ========================================
   ...
   Replace your renderUsersTable function with:
   
   function renderUsersTable(usersList) {
     return renderResponsiveTable(
       usersList,
       renderUsersTableDesktop,  // Your existing table renderer
       renderUsersMobileCards     // New mobile card renderer
     );
   }
   ======================================== */
